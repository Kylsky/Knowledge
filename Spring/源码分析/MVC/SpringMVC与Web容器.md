...
initWebApplicationContext方法中比较重要的就是**cwac.setParent(rootContext)**和**getServletContext().setAttribute(attrName, wac)**这两部分，简单点来说是绑定了双亲上下文，使根上下文与子上下文的spring容器产生父子关系，关于父子容器这里不详细展开，之前IoC的文章有分析过，也可以参考下面的文章：
